...
//		panelStatistics.setBackground(new Color(243, 202, 82));
		...
		todayPanel.setBorder(new TitledBorder(null, "<html><div style=\"font-size:16px\">Employees On Leave : TODAY",
		...
		weekPanel.setBorder(new TitledBorder(null, "<html><div style=\"font-size:16px\">Employees On Leave : THIS WEEK",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		monthPanel
				.setBorder(new TitledBorder(null, "<html><div style=\"font-size:16px\">Employees On Leave : THIS MONTH",
						TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		holidaysArea = new JTextArea(
				"Fri 2020/10/23 - Wed 2020/10/28\nDashain\nSat 2020/11/14 - Mon 2020/11/16\nTihar\nFri 2021/01/01\nEnglish New Year\nThu 2021/03/11\nShivaratri\nSun 2021/03/28\nHoli");
	...
	private void showEmployeeDetails(JButton button, String str) {
		...
		var dao = new LeaveRequestDao();
		dao.selAllLeave(str).stream()
				.forEach(e -> model.addRow(new Object[] { e.getEmployeeId(), e.getLeaveTypeId(),
						daoEmp.getEmp(e.getEmployeeId()).getEmployeeName(),
						daoLt.selectLeaveTypeById(e.getLeaveTypeId()).getLeaveTypeName(), e.getStartDate(),
						e.getEndDate(), e.getReason() }));
